...
const GltfModel = ({ url, mousePosition, scrollPosition }) => {
...
const ThreeDScene = ({scrollPosition,mousePosition}) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh',pointerEvents: 'none' }}>
      <Canvas id="canvas-container" style={{ position: 'absolute', top: 0, left: 0, zIndex: 2,pointerEvents: 'none' }} camera={{ position: [0, 1.35, 5], fov: 7 }}>
        <ambientLight intensity={0.5} />
        <directionalLight position={[10, 10, 10]} intensity={3} />
        <GltfModel url="/w.glb" scrollPosition={scrollPosition} mousePosition={mousePosition} />
        <OrbitControls enablePan={false} enableZoom={false} enableRotate={false} enabled={false}/>
      </Canvas>
    </div>
  );
};
